--[[
	SkyCamera - Copyright (c) 2023 Dottik
	...
	TODO:
		...
		- Movement with more than 1 key [INCOMPLETE]
]]
...
local LoggerModule = require(game:GetService("ReplicatedStorage").Shared.LoggerModule) -- Logger Module
...
local function MoveCamera(camera: Camera, direction: number, bePerpendicular: boolean)
	...
	local finalVector = (bePerpendicular and oldCameraCframe.LookVector:Cross(Vector3.new(0, 1, 0)))
		or oldCameraCframe.LookVector
	local newPosition = oldCameraCframe.Position + finalVector * direction * CameraSpeed

	local diff = math.min(newPosition.Y, oldCameraCframe.Position.Y)
		- math.max(newPosition.Y, oldCameraCframe.Position.Y)
	local finalY = not bePerpendicular
			and direction == -1
			and (math.max(newPosition.Y, diff) + math.min(newPosition.Y, diff))
		or not bePerpendicular and direction == 1 and (newPosition.Y - diff)
		or math.max(newPosition.Y, diff) + math.min(newPosition.Y, diff)
	...
	if IsCFrameInBoundingBox(targetCframe, MovementZone) then
		...
		local tween =
			TweenService:Create(camera, TweenInfo.new(0.2, Enum.EasingStyle.Circular), { CFrame = targetCframe })
		tween:Play()
		workspace.CurrentCamera.CameraType = Enum.CameraType.Scriptable
	end
end
...
local function RotateCamera(camera: Camera, rotationAngleY: number)
	...
	if IsCFrameInBoundingBox(targetCframe, MovementZone) then
		workspace.CurrentCamera.CameraType = Enum.CameraType.Scriptable
		local tween =
			TweenService:Create(camera, TweenInfo.new(0.4, Enum.EasingStyle.Circular), { CFrame = targetCframe })
		tween:Play()
		workspace.CurrentCamera.CameraType = Enum.CameraType.Scriptable
	end
end
...
UserInputService.InputChanged:Connect(function(input: InputObject, gameProcessedEvent: boolean)
	...
	if input.UserInputType == Enum.UserInputType.MouseWheel then
		...
		if input.Position.Z > 0 then
			newPosition = workspace.CurrentCamera.CFrame.Position
				+ workspace.CurrentCamera.CFrame.LookVector * ZoomFactor
